...
REDUCED="/network/projects/_groups/musai/data/ml-1m/reduced/features/"
...
for k in 4 5 ; do \
    for i in 10 30 50 70 90 ; do \
    FEAT_K=${REDUCED}${k}_${i}.txt
    if [[ ! -f $FEAT_K ]]
    then
        python reduce_features.py -f $FEAT -p $i > $FEAT_K
    fi
    mkdir -p $RESULTS/reduced_$k/$i/
    mkdir -p $RESULTS_FAIR/reduced_$k/$i/
    mkdir -p $RESULTS_DIV/reduced_$k/$i/
    ...
    for f in $RECS*.gz ; do 
        ...
        java -cp "../target/community-1.jar:../target/dependency/*" com.mila.recsys.MetricExample $TRAIN /tmp/gt.txt $FEAT_K /tmp/$FNAME 4 > $RESULTS_DIV/reduced_$k/$i/$FNAME.txt;
        python -u commonality.py -rec /tmp/$FNAME -feat $FEAT_K -gamma "0.5" > $RESULTS/reduced_$k/$i/$FNAME.txt
        python -u fairness.py -rec /tmp/$FNAME -feat $FEAT_K -rel $REL -train $TRAIN > $RESULTS_FAIR/reduced_$k/$i/$FNAME.txt
        #rm /tmp/file1.txt
        fi
        done
    done
