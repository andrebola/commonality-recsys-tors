...
def compare_all_metrics(agg_metrics, univ_metric):
    ...
    for metric in agg_metrics:
        for red in sorted(agg_metrics[metric].keys()):
            for it in [1, 2, 3, 4, 5]:
                x_dict = {k:agg_metrics[metric][red][k][it-1] for k in agg_metrics[metric][red]}
                y_dict = {k:univ_metric[red][k][it-1] for k in univ_metric[red]}
 
                vals_x, vals_y, names = metric_pairs(x_dict, y_dict)
                #print (metric, red, agg_metrics[metric][red], univ_metric[red] ,vals_x, vals_y)
                c,p = kendalltau(vals_x, vals_y)
                if red not in ret :
                     ret[red] = {}
                if metric not in ret[red]:
                    ret[red][metric] = []
                ret[red][metric].append(c)
    for red in ret:
        for m in ret[red]:
           ret[red][m] = (np.mean(ret[red][m]), np.std(ret[red][m]))
    return ret
...
if __name__ == '__main__':
    ...
    metrics.update(red_fairness_metric)
    ...
    metrics.update(red_div_metric)

    # Commonality:
    folder_univ = "/network/projects/_groups/musai/results/comm-gamma5-ml-1m-feat-red/"
    red_comm_borda, red_comm_metric = get_files_comm_reduced(folder_univ, red_folder=red_folder)

    metrics_comp = compare_all_metrics(metrics, red_comm_borda)

    for metric in list(metrics_comp.values())[0].keys():
        for red in metrics_comp:
            v = metrics_comp[red][metric]
            print ("BORDA",",", red,"," ,str(metric),",",str(v[0]),",",str(v[1]))

    metrics_comp = compare_all_metrics(metrics, red_comm_metric)

    for metric in list(metrics_comp.values())[0].keys():
        for red in metrics_comp:
            v = metrics_comp[red][metric]
            print ("MEAN",",", red,"," ,str(metric),",",str(v[0]),",",str(v[1]))
